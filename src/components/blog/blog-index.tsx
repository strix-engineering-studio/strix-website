          ...
          {tags.map((item) => (
            <button
              ...
              className={cn(
                ...
                tag === item ? "border-white/20 bg-white/10 text-foreground" : "border-white/10 bg-white/5 text-white/68 hover:bg-white/8"
              )}
            >
      ...
      {filtered[0] ? (
        <Link href={`/blog/${filtered[0].slug}`} className="group grid overflow-hidden rounded-xl border border-white/10 bg-white/4 lg:grid-cols-[0.95fr_1.05fr]">
          ...
          <div className="space-y-4 p-5 sm:p-6 lg:p-8">
            ...
            <h2 className="text-3xl font-semibold tracking-tight text-foreground sm:text-4xl">{filtered[0].title}</h2>
            <p className="max-w-2xl text-sm leading-7 text-muted-foreground">{filtered[0].excerpt}</p>
          ...
          <Link key={post.slug} href={`/blog/${post.slug}`} className="group overflow-hidden rounded-xl border border-white/10 bg-white/4 transition hover:-translate-y-1 hover:bg-white/5">
            ...
            <div className="space-y-4 p-5">
              <div className="flex items-center justify-between gap-4">
                <p className="text-xs uppercase tracking-[0.35em] text-white/45">{post.tag}</p>
              ...
              <p className="max-w-2xl text-sm leading-7 text-muted-foreground">{post.excerpt}</p>
