...
const budgetOptions = ["Under ₹25k", "₹25k–₹75k", "₹75k–₹2L", "₹2L–₹5L", "₹5L+"]
...
export function ProjectInquiryForm() {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      importantRequirements: String(formData.get("importantRequirements") || ""),
    ...
    <form onSubmit={onSubmit} className="space-y-8 rounded-xl border border-white/10 bg-white/4 p-5 sm:p-6 lg:p-7">
      <Section title="Contact">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <TextField label="Email Address" name="emailAddress" placeholder="you@company.com" type="email" required icon={<Mail className="size-4 text-white/40" />} />
          <TextField label="WhatsApp Number" name="whatsappNumber" placeholder="+91 98765 43210" type="tel" required icon={<Phone className="size-4 text-white/40" />} />
          <TextField label="Company / Team Name" name="companyName" placeholder="Organization or team" />
          <TextField label="Website / Product Link" name="websiteProductLink" placeholder="https://..." />
        </div>
      </Section>
      ...
      <Section title="Project Overview">
        <div className="space-y-4">
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <RadioField label="Project Type" name="projectType" options={projectTypes} required />
            <RadioField label="Current Stage" name="currentStage" options={currentStages} required />
            <RadioField label="Do you already have designs or references?" name="designReadiness" options={designReadinessOptions} />
            <TextField label="Reference Links (optional)" name="referenceLinks" placeholder="Competitors, Dribbble, Loom, existing app" />
          </div>
        </div>
      </Section>
      ...
      <Section title="Budget & Timeline">
        <div className="grid gap-4 lg:grid-cols-3">
          <RadioField label="Estimated Budget" name="estimatedBudget" options={budgetOptions} required />
          <RadioField label="Expected Timeline" name="expectedTimeline" options={timelineOptions} required />
          <RadioField label="Are you the decision maker?" name="decisionMaker" options={decisionMakerOptions} />
        </div>
      </Section>
      ...
      <Section title="Final">
        <div className="space-y-4">
          ...
          <TextareaField label="Anything else you'd like to share?" name="anythingElse" rows={5} placeholder="Anything useful for scoping or fit." />
        </div>
      </Section>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <p className="max-w-xl text-sm leading-6 text-white/52">
          I review structured inquiries personally. If the project is a fit, I’ll respond with next steps, questions, or a discovery call.
        </p>
      </div>
      ...
      {status === "success" ? (
        <p className="flex items-center gap-2 text-sm text-foreground">
          ...
          Thanks for submitting your project inquiry. I’ll review the details and reach out if the project aligns well.
        </p>
      ) : null}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="space-y-4">
      <p className="text-sm uppercase tracking-[0.28em] text-emerald-300/75">{title}</p>
      {children}
    </section>
  )
}

function TextField({ label, icon, ...props }: React.InputHTMLAttributes<HTMLInputElement> & { label: string; icon?: React.ReactNode }) {
  return (
